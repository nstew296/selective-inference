...
def test_lasso(s=5, n=200, p=20, Langevin_steps=10000, burning=2000,
    ...
    #print "active weight", active_weight
    #print "lam", lam
    #print penalty.lagrange
    init_state[subgrad_slice] = (-initial_grad[inactive] + random_Z[inactive] * randomization_scale) / (inactive_weight * penalty.lagrange)

    #print "init sub ", init_state[subgrad_slice]
    #print (-initial_grad[inactive] + random_Z[inactive] * randomization_scale) /lam
    ...
    # quadratic part
    linear_term[active][:,beta_slice] = linear_term[active][:,beta_slice] + epsilon * np.identity(nactive)

    # subgrad part
    linear_term[inactive][:,subgrad_slice] = linear_term[inactive][:,subgrad_slice] + np.diag(penalty.lagrange * inactive_weight)
