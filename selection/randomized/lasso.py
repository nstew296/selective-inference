...
class lasso(gaussian_query):
    ...
    def fit(self,
        ...
        #print("check subgrad ", self.initial_subgrad, active.sum())
        ...
        self.observed_opt_state = np.concatenate([initial_unpenalized,
                                                  initial_scalings])
        print("observed_opt_state ", self.observed_opt_state)
        ...
        _score_linear_term = -np.hstack([_hessian_unpen, _hessian_active])
        ...
        #print("check opt_offset", opt_offset[0], opt_offset[active], opt_offset[unpenalized], unpenalized)
        ...
        # if (A_scaling[:active.sum()].dot(self.observed_opt_state) -b_scaling[:active.sum()]<0).sum()-active.sum()>0:
        #     STOP
        if (A_scaling[1:].dot(self.observed_opt_state) -b_scaling[1:]<0).sum()-active.sum()>0:
            STOP
        ...
        self._setup_sampler_data = (A_scaling[1:],
                                    b_scaling[1:],
                                    opt_linear,
                                    opt_offset)
