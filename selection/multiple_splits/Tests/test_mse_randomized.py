...
def test_mse(n=200, p=1000, nval=200, alpha=2., rho=0.70, s=10, beta_type=1, snr=1.0,
    ...
    for i in range(nsim):
        ...
        X -= X.mean(0)[None, :]
        y = y - y.mean()
        OLS_weights = np.fabs(X.T.dot(y))


        X /= (X.std(0)[None, :] * np.sqrt(n / (n - 1.)))
        dispersion = None
        sigma_ = np.std(y)
        lam_theory = sigma_ * 0.60 *  np.mean(np.fabs(np.dot(X[:, 1:].T, np.random.standard_normal((n, 2000)))).max(0))
        #HOLP = np.abs(X.T.dot(np.linalg.inv(X.dot(X.T)+ 0.5)).dot(y))
        ...
        for j in range(B):
            lasso_sol = lasso.gaussian(X,
                                       ...
                                       feature_weights=np.append(0.001, (n*n)/OLS_weights[1:]),
                                       randomizer_scale=np.sqrt(n) * randomizer_scale * sigma_)
            signs = lasso_sol.fit()
            nonzero = signs != 0
            #print("check ", np.diag(X.T.dot(X)), nonzero.sum(), (np.sqrt(n)*n)/OLS_weights[1:], lam_theory)
            ...
            HOLP_split = np.abs(X_sel.T.dot(np.linalg.inv(X_sel.dot(X_sel.T)+0.5)).dot(y_sel))

            lam_theory_split = sigma_ * 0.60 * np.mean(np.fabs(np.dot(X_sel[:, 1:].T, np.random.standard_normal((subsample_size, 2000)))).max(0))
            lasso_split = lasso_full.gaussian(X_sel, y_sel, np.append(0.001, np.ones(p - 1) * split_fraction * (n*n)/OLS_weights[1:]))
            #lasso_split = lasso_full.gaussian(X_sel, y_sel, np.append(0.001, 1./HOLP_split[1:]))
            lasso_soln = lasso_split.fit()
            active_LASSO = (lasso_soln != 0)
            nactive_LASSO = active_LASSO.sum()
            print("check SPLIT", nactive_LASSO)
