...
def test_mse(n=200, p=1000, nval=200, alpha= 2., rho=0.70, s=10, beta_type=1, snr=1.0, randomizer_scale=1., split_fraction=0.67,
    ...
    mse_split = 0.
    mse_debiased = 0.
    bias_weight_debiased = 0.
    mse_weight_debiased = 0.
    ...
    for i in range(nsim):
        ...
        alpha_target_debiased = np.zeros(B)
        alpha_target_split = np.zeros(B)

        sel_mle = np.zeros(B)
        debiased_sel_mle = np.zeros(B)
        est_split = np.zeros(B)
        weight_debiased = np.zeros(B)
        for j in range(B):
            lasso_sol = lasso.gaussian(X,
                                       y,
            ...
            if nonzero[0] == 1:
                ...
                debiased_observed_target_uni = (debiased_observed_target[0]).reshape((1,))
                debiased_cov_target_uni = (np.diag(debiased_cov_target)[0]).reshape((1, 1))
                debiased_cov_target_score_uni = debiased_cov_target_score[0, :].reshape((1, p))

                debiased_mle, var_mle, _, _, _, _, _, _ = lasso_sol.selective_MLE(debiased_observed_target_uni,
                                                                                  debiased_cov_target_uni,
                                                                                  debiased_cov_target_score_uni,
                                                                                  alternatives)

                debiased_sel_mle[j] = debiased_mle
                weight_debiased[j] = 1./var_mle
        ...
        alpha_target_debiased = alpha_target_debiased[alpha_target_debiased != 0]
        sel_mle = sel_mle[sel_mle !=0]
        debiased_sel_mle = debiased_sel_mle[debiased_sel_mle != 0]
        indices = np.argsort(weight_debiased)
        weights = np.arange(B)[indices] + 1.
        print("check weights ", weight_debiased, weights/weights.sum())
        weight_debiased_sel_mle = (weights/weights.sum()) * debiased_sel_mle
        ...
        avg_target_debiased = np.mean(alpha_target_debiased)
        bias_tar_debiased += (avg_target_debiased - alpha)
        bias_debiased += (np.mean(debiased_sel_mle) - alpha)
        mse_debiased += ((np.mean(debiased_sel_mle) - alpha) ** 2)

        bias_weight_debiased += (weight_debiased_sel_mle.sum() - alpha)
        mse_weight_debiased += ((weight_debiased_sel_mle.sum() - alpha) ** 2)
        ...
        print("bias in target so far ", bias_tar_randomized/float(i+1.), bias_tar_split/float(i+1.), bias_tar_debiased/float(i+1.))
        print("bias in estimate so far ", bias_randomized / float(i + 1.), bias_split / float(i + 1.), bias_debiased/float(i+1.), bias_weight_debiased/float(i+1.))
        print("mse so far ", mse_randomized/ float(i + 1.), mse_split/ float(i + 1.), mse_debiased/float(i+1.), mse_weight_debiased/float(i+1.))

        print("iteration completed ", i)
    bias_tar_randomized/=float(nsim)
    bias_tar_split/=float(nsim)
    bias_tar_debiased /=float(nsim)
    bias_randomized/=float(nsim)
    mse_randomized/=float(nsim)
    bias_debiased/=float(nsim)
    mse_debiased/=float(nsim)
    bias_split/=float(nsim)
    mse_split/=float(nsim)
    bias_weight_debiased/=float(nsim)
    mse_weight_debiased/=float(nsim)
...
def output_file(n=200, p=1000, nval=200, alpha= 2., rho=0.35, s=10, beta_type=0, snr=0.71,
    ...
    for B_agg in Bval:
        ...
        df_mse_B = pd.DataFrame(data=output.reshape((1, 11)),
                                columns=['bias_tar_randomized', 'bias_tar_split', 'bias_tar_debiased',
                                         'bias_randomized', 'bias_split', 'bias_debiased', 'bias_weight_debiased',
                                         'mse_randomized', 'mse_split', 'mse_debiased', 'mse_weight_debiased'])
    ...
    outfile_inf_html = os.path.join(outpath, "dims_" + str(n) + "_" + str(p) + "_mse_compare_Dnormal_" + str(beta_type) +  "_rho_" + str(rho) + ".html")
    outfile_inf_csv = os.path.join(outpath,"dims_" + str(n) + "_" + str(p) + "_mse_compare_Dnormal_" + str(beta_type) + "_rho_" + str(rho) + ".csv")
