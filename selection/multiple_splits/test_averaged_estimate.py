...
def test_lasso_estimate(X, y, sigma, beta,  OLS_weights, weights= "theory", randomizer_scale = 1.):
    ...
    while True:
        ...
        n, p = X.shape
        if weights=="theory":
            lam = np.ones(p - 1) * sigma_ * np.mean(np.fabs(np.dot(X[:,1:].T, np.random.standard_normal((n, 2000)))).max(0))
        else:
            lam = (n*n)/OLS_weights[1:]
        ...
        if nonzero.sum()>0:
            ...
            print("target and estimate", mle, beta_target[0])
            return np.asscalar(mle)
...
def main(n=200, p=1000, nval=200, alpha= 2., rho=0.70, s=10, beta_type=1, snr=0.20, randomizer_scale=0.5, nsim=100, B=10):
    ...
    for i in range(nsim):
        ...
        estimate = 0.
        for b in range(B):
            est= test_lasso_estimate(X, y, sigma, beta, OLS_weights, randomizer_scale=randomizer_scale, weights=None)
            estimate += est
        carved_est.append(estimate/float(B))
        bias += (estimate/float(B)- beta[0])
        mse += ((estimate/float(B) - beta[0])**2)
        print("iteration completed ", i, '\n')
...
main(n=100, p=500, nval=100, alpha= 2., rho=0.35, s=5, beta_type=1, snr=0.71, randomizer_scale=1., nsim=1000, B=3)
