...
print(np.corrcoef(np.vstack((Y1,Y2,Y3,Y4,Y5,Y6,Y7,Y8,Y9,Y10,Y11))))
...
def rand_multi_task_selection_inference(predictor_vars_train,predictor_vars_validate,predictor_vars_test,response_train,
    ...
    cov = np.zeros((ntask * sample_sizes, ntask * sample_sizes))
    noise_levels = []
    ...
    for i in range(ntask):
        ...
        cov[i * sample_sizes:(i + 1) * sample_sizes, i * sample_sizes:(i + 1) * sample_sizes] = (noise_levels[i] ** 2) * np.identity(sample_sizes)

        for j in tasks[tasks < i]:
            cov_ij = (1. / (sample_sizes - nfeatures)) * np.inner(response_train[i] - (predictor_vars_train).dot(
            np.linalg.pinv((predictor_vars_train)).dot(response_train[i])),response_train[j] - (predictor_vars_train).dot(
            np.linalg.pinv((predictor_vars_train)).dot(response_train[j])))
            cov[sample_sizes * i: sample_sizes * (i + 1):, sample_sizes * j:sample_sizes * (j + 1)] = cov_ij * np.identity(sample_sizes)
    ...
    for weight in weight_list:
        ...
        estimate, observed_info_mean, Z_scores, pvalues, intervals = multi_lasso.multitask_inference_hetero(precision)
...
def ds_multi_task_selection_inference(predictor_vars_selection,predictor_vars_inference,predictor_vars_validate,
    ...
    cov = np.zeros((ntask * sample_sizes_inference, ntask * sample_sizes_inference))
    noise_levels = []
    tasks = np.arange(ntask)
    for i in range(ntask):
       ...
       cov[i * sample_sizes_inference:(i + 1) * sample_sizes_inference, i * sample_sizes_inference:(i + 1) * sample_sizes_inference] = (noise_levels[i] ** 2) * np.identity(sample_sizes_inference)

       for j in tasks[tasks < i]:
            cov_ij = (1. / (sample_sizes_inference - nfeatures)) * np.inner(response_inference[i] - (predictor_vars_inference).dot(
            np.linalg.pinv((predictor_vars_inference)).dot(response_inference[i])),response_inference[j] - (predictor_vars_inference).dot(
            np.linalg.pinv((predictor_vars_inference)).dot(response_inference[j])))
            cov[sample_sizes_inference * i: sample_sizes_inference * (i + 1):, sample_sizes_inference * j:sample_sizes_inference * (j + 1)] = cov_ij * np.identity(sample_sizes_inference)
    ...
    for weight in weight_list:
        ...
        if (active_signs != 0).sum() > 0:
            ...
            estimate = cov_targets.dot(selected.T.dot(precision).dot(response))

            CV = np.sqrt(np.diag(cov_targets)) / np.abs(estimate)
            alpha = 1. - 0.90
            quantile = ndist.ppf(1 - alpha / 2.)
            intervals = np.vstack([estimate - quantile * np.sqrt(np.diag(cov_targets)),
                                   estimate + quantile * np.sqrt(np.diag(cov_targets))]).T
            ...
            for j in range(ntask):
                ...
                if idx_new == 0:
                    ...
                    error += np.sqrt(np.sum(
                        np.square(
                            (response_validate[j] - (predictor_vars_validate)[:, (active_signs[:, j] != 0)].dot(
                                estimate[idx:idx + idx_new])))) / sample_sizes_validate)
                idx = idx + idx_new
...
final_estimates_rand1, final_intervals_rand1, selective1_intervals, all_variables_rand1, significant_variables_rand1, final_err_rand1, pred_r_rand1, coefs_var_rand1 = \
    rand_multi_task_selection_inference(predictors_train,predictors_validate,predictors_test, responses_train,
                                        responses_validate, responses_test,weight_list = np.arange(44,55,0.3),rand_scale=1.0)
