...
class multi_task_lasso():
    ...
    def fit(self,
        ...
        for i in range(self.ntask):
            ...
            _observed_score_state[self.inactive[i]] += self.loglikes[i].smooth_objective(beta_bar[:, i], 'grad')[
                self.inactive[i]]
            observed_score_state[i] = _observed_score_state

            active_directions = np.array([signed_basis_vector(p,
                                                              k,
                                                              (active_signs[:, i])[k]) for k in
                                          np.nonzero(active[:, i])[0]]).T
    ...
    def _solve_multitask_problem(self, perturbations=None, num_iter=1000, atol=1.e-4):
    ...
    def gaussian(predictor_vars,
        ...
        loglikes = {
            i: rr.glm.gaussian(predictor_vars[i], response_vars[i], coef=1. / noise_levels[i] ** 2, quadratic=quadratic)
