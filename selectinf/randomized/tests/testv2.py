...
def cross_validate_posi_global(ntask=2,
    ...
    samples = np.arange(nsamples[0])

    for i in range(10):
        folds[i] = np.random.choice(samples, size=np.int(np.round(.1*nsamples[0])), replace=False)
        samples = np.setdiff1d(samples, folds[i])

    lambdamin = 0.5
    lambdamax = 2 * np.sqrt(2 * np.log(p))
    weights = np.arange(np.log(lambdamin), np.log(lambdamax), lambdamax / 25)
    ...
    for i in range(10):
        ...
        train = np.setdiff1d(np.arange(nsamples[0]),folds[i])
        test = folds[i]
...
def cross_validate_naive_global(ntask=2,
                   nsamples=500 * np.ones(2),
                   p=100,
                   global_sparsity=.8,
                   task_sparsity=.3,
    ...
    for i in range(10):
        ...
        for w in range(len(weights)):
            ...
            try:
                ...
                for j in range(ntask):
                    observed_target_.extend(
                        np.linalg.pinv((predictor_vars_train[j])[:, multi_lasso.active_global]).dot(response_vars_train[j]))
            ...
            for j in range(ntask):
                ...
                error.extend(response_vars_test[j] - (predictor_vars_test[j])[:, multi_lasso.active_global].dot(observed_target))
...
def test_coverage(sparsity,nsim=100):
    cov = []
    len = []
    pivots = []
    ...
    penalty = cross_validate_posi_global(ntask=ntask,
                            ...
                            task_sparsity=sparsity,
                            sigma=1. * np.ones(ntask),
                            signal_fac=0.7,
                            rhos=0.1 * np.ones(ntask))


    penalty_naive = cross_validate_naive_global(ntask=ntask,
                                         nsamples=1000 * np.ones(ntask),
                                         p=50,
                                         global_sparsity=0.95,
                                         task_sparsity=sparsity,
    ...
    for n in range(nsim):
        ...
        try:
            ...
            coverage_naive, length_naive, pivot_naive = test_multitask_lasso_naive_global(ntask=ntask,
                                                                  nsamples=1000 * np.ones(ntask),
                                                                  p=50,
                                                                  global_sparsity=0.95,
                                                                  task_sparsity=sparsity,
    ...
    #points_naive = [np.searchsorted(np.sort(np.asarray(pivots_naive)), i,side='right') / np.float(np.shape(pivots_naive)[0]) for i in
              #np.linspace(0, 1, 101)]
    #plt.plot(grid, points, c='blue', marker='^')
    #plt.plot(grid, points_naive, c='red', marker='^')
    #plt.plot(grid, grid, 'k--')
    #plt.title('Empirical Distribution of Pivots')
    #plt.show()
...
def main():
    ...
    for i in range(len(sparsity)):
        print(sparsity[i], 'signal')
        results = test_coverage(sparsity[i], nsim=100)
        coverage = np.append(coverage, results[0])
        print(coverage,"cov")
        coverage_er = np.append(coverage_er, 1.64 * results[1] / np.sqrt(100))
        lengths = np.append(lengths, results[2])
        lengths_er = np.append(lengths_er, 1.64 * results[3] / np.sqrt(100))

    fig = plt.figure()
    plt.errorbar(sparsity, coverage, yerr=coverage_er)
    axes = plt.gca()
    axes.set_ylim([0, 1])
    plt.ylabel('Mean Coverage')
    plt.xlabel('Task Sparsity')
    plt.title('Coverage by Task Sparsity')
    plt.savefig("coverage.png")
    plt.show()

    fig = plt.figure()
    plt.errorbar(sparsity, lengths, yerr=lengths_er)
