...
def test_single_task_lasso_posi_hetero(predictor_vars_train,
    ...
    sensitivity_inference = np.float(num_true_positive_inference) / np.float(num_positive)
    specificity_inference = 1.0 - np.float(num_false_positive_inference) / np.float(num_negative)
    print(sensitivity_inference,"test")
    print(specificity_inference,"test")
...
def test_coverage(weight,signal,nsim=100):
    ...
    for n in range(nsim):
        ...
        print("error ds", np.median(np.asarray(ds_error)))
...
def main():
    ...
    for i in range(len(feature_weight_list)):
        sims = test_coverage(feature_weight_list[i],[0.5,1.0],200)
    ...
    plt.savefig('lengthcompare_weak.png',bbox_inches='tight')
    ...
    plt.savefig('coveragecompare_weak.png',bbox_inches='tight')
    ...
    plt.savefig('specificitycompare_weak.png')
    ...
    plt.savefig('errcompare_weak.png',bbox_inches='tight')
