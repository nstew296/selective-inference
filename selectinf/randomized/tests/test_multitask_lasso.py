...
from selectinf.tests.instance import gaussian_multitask_instance, gaussian_multitask_instance_v2, \
...
def test_multitask_lasso_selective_inference(predictor_vars_train,
    ...
    try:
        ...
        print("No active predictors for any task - received index error ", e)
...
def test_separate_multitask_lasso_selective_inference(predictor_vars_train,
                                                      response_vars_train,
                                                      predictor_vars_test,
                                                      response_vars_test,
                                                      beta,
    ...
    try:
        ...
        print("multi-task lasso fitting error - group 1", e)
    ...
    try:
        ...
        print("multi-task lasso fitting error - group 2", e)
    ...
    if (active_signs != 0).sum() > 0:
        ...
        for i in range(ntask):
            X, y = predictor_vars_train[i], response_vars_train[i]
            beta_target.extend(np.linalg.pinv(X[:, (active_signs[:, i] != 0)]).dot(X.dot(beta[:, i])))
...
def test_single_task_lasso_selective_inference(predictor_vars_train,
    ...
    for i in range(ntask):
        ...
        try:
            ...
            print("single-task lasso error", e)
            pass
...
def test_inference_mixed_ts(weight, signal, p, ts, gs, nsim=100, seed=5):
    ...
    response_vars_train, predictor_vars_train, response_vars_test, predictor_vars_test, beta, gaussian_noise = \
        gaussian_multitask_instance_two_ts(ntask,
                                           nsamples,
                                           nsamples_test,
                                           p,
                                           global_sparsity,
            ...
            for i in range(ntask):
                if i == 0:
                    ...
                    response_vars_test[i] = predictor_vars_test[i].dot(beta[:, i])\
                        + (gaussian_noise[nsamples.sum():nsamples.sum() + nsamples_test_cumsum[i]]) * sigma[i]
                else:
                    response_vars_train[i] = predictor_vars_train[i].dot(beta[:, i])\
                        + (gaussian_noise[nsamples_train_cumsum[i - 1]:nsamples_train_cumsum[i]]) * sigma[i]
                    response_vars_test[i] = predictor_vars_test[i].dot(beta[:, i])\
                        + (gaussian_noise[nsamples.sum() + nsamples_test_cumsum[i - 1]:
...
def test_inference_one_vs_two_models(weight, signal, p, structure, gs, nsim=100, seed=5):
    np.random.seed(seed)
    # Track intervals, pivots, sensitivity, specificity, and testing error for joint MTL + SI
    cov, len, pivots, sensitivity_list, specificity_list, test_error_list = ([] for _ in range(6))
    ...
    response_vars_train, predictor_vars_train, response_vars_test, predictor_vars_test, beta, gaussian_noise = \
        gaussian_multitask_instance_v2(ntask,
                                       nsamples,
                                       nsamples_test,
                                       p,
                                       global_sparsity,
    ...
    for n in range(nsim):
        ...
        coverage_separate, length_separate, sns_separate, spc_separate, err_separate = \
            test_separate_multitask_lasso_selective_inference(
    ...
    return ({"joint_coverage": np.asarray(cov),
             "separate_coverage": np.asarray(cov_separate),
             "joint_length": np.asarray(len),
             "separate_length": np.asarray(len_separate),
             "joint_sensitivity": np.asarray(sensitivity_list),
             "separate_sensitivity": np.asarray(sensitivity_list_separate),
             "joint_specificity": np.asarray(specificity_list),
             "separate_specificity": np.asarray(specificity_list_separate),
             "joint_error": np.mean(np.asarray(test_error_list)),
...
def test_inference_error_comparison(weight, signal, p, ts, gs, nsim=100, seed=5):
    ...
    for n in range(nsim):
        ...
        for i in range(ntask):
            if i == 0:
                gaussian_response_vars_train[i] = predictor_vars_train[i].dot(beta[:, i]) + \
                    (gaussian_noise[:nsamples_train_cumsum[i]]) * sigma[i]
                gaussian_response_vars_test[i] = predictor_vars_test[i].dot(beta[:, i]) +\
                    (gaussian_noise[nsamples.sum():nsamples.sum() + nsamples_test_cumsum[i]]) * sigma[i]
                exponential_response_vars_train[i] = predictor_vars_train[i].dot(beta[:, i]) + \
                    (exponential_noise[:nsamples_train_cumsum[i]]) * sigma[i]
                exponential_response_vars_test[i] = predictor_vars_test[i].dot(beta[:, i]) + \
                    (exponential_noise[nsamples.sum():nsamples.sum() + nsamples_test_cumsum[i]]) * sigma[i]
                laplace_response_vars_train[i] = predictor_vars_train[i].dot(beta[:, i]) + \
                    (laplace_noise[:nsamples_train_cumsum[i]]) * sigma[i]
                laplace_response_vars_test[i] = predictor_vars_test[i].dot(beta[:, i]) + \
                    (laplace_noise[nsamples.sum():nsamples.sum() + nsamples_test_cumsum[i]]) * sigma[i]
            else:
                gaussian_response_vars_train[i] = predictor_vars_train[i].dot(beta[:, i]) + \
                    (gaussian_noise[nsamples_train_cumsum[i - 1]:nsamples_train_cumsum[i]]) * sigma[i]
                gaussian_response_vars_test[i] = predictor_vars_test[i].dot(beta[:, i]) + \
                    (gaussian_noise[nsamples.sum() + nsamples_test_cumsum[i - 1]:
                                    nsamples.sum() + nsamples_test_cumsum[i]]) * sigma[i]
                exponential_response_vars_train[i] = predictor_vars_train[i].dot(beta[:, i]) + \
                    (exponential_noise[nsamples_train_cumsum[i - 1]: nsamples_train_cumsum[i]]) * sigma[i]
                exponential_response_vars_test[i] = predictor_vars_test[i].dot(beta[:, i]) + \
                    (exponential_noise[nsamples.sum() + nsamples_test_cumsum[i - 1]:
                                       nsamples.sum() + nsamples_test_cumsum[i]]) * sigma[i]
                laplace_response_vars_train[i] = predictor_vars_train[i].dot(beta[:, i]) + \
                    (laplace_noise[nsamples_train_cumsum[i - 1]:nsamples_train_cumsum[i]]) * sigma[i]
                laplace_response_vars_test[i] = predictor_vars_test[i].dot(beta[:, i]) + \
                    (laplace_noise[nsamples.sum() + nsamples_test_cumsum[i - 1]:
                                   nsamples.sum() + nsamples_test_cumsum[i]]) * sigma[i]
