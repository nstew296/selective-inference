...
plt.sca(ax1)
second = plt.boxplot([coverage_by_p[j][0] for j in range(len(p_list))], positions=np.array(range(length)) * 3 , sym='', widths=0.3)
third = plt.boxplot([coverage_by_p[j][1] for j in range(len(p_list))], positions=np.array(range(length)) * 3 +.3, sym='', widths=0.3)
fourth = plt.boxplot([coverage_by_p[j][3] for j in range(len(p_list))], positions=np.array(range(length)) * 3 + .6, sym='', widths=0.3)
fifth = plt.boxplot([coverage_by_p[j][4] for j in range(len(p_list))], positions=np.array(range(length)) * 3 + 0.9, sym='', widths=0.3)
sixth = plt.boxplot([coverage_by_p[j][5] for j in range(len(p_list))], positions=np.array(range(length)) * 3 + 1.2, sym='',widths=0.3)
seventh = plt.boxplot([coverage_by_p[j][6] for j in range(len(p_list))], positions=np.array(range(length)) * 3 + 1.5, sym='',widths=0.3)
